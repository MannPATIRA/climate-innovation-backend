...
def neo4j_operation_with_retry(max_retries=3, retry_delay=60):
    ...
    def decorator(func):
        @wraps(func)
        async def wrapper(self: 'AsyncNeo4jClient', *args, **kwargs):
            retries = 0
...
class AsyncNeo4jClient:
    ...
    async def initialize(self):
        """Initialize the client by ensuring constraints and indexes"""
        await self._ensure_constraints_and_indexes()
    ...
    def restart_neo4j_service(self) -> bool:
        ...
        try:
            ...
            AsyncNeo4jClient._restart_lock.release()

    async def close(self):
        await self.driver.close()

    @neo4j_operation_with_retry()
    async def _ensure_constraints_and_indexes(self):
    ...
    @neo4j_operation_with_retry()
    async def merge_paper_node(self, paper_id: str, title: str, year: Optional[int], citations: int = 0):
        async with self.driver.session() as session:
            query = """
            ...
            """
            await session.run(query, paper_id=paper_id, title=title, year=year, citations=citations)

    @neo4j_operation_with_retry()
    async def merge_author_paper_relationship(self, author_id: str, paper_id: str, 
    ...
    @neo4j_operation_with_retry()
    async def merge_paper_topic_relationship(self, paper_id: str, topic_id: str, topic_name: str, score: float):
        async with self.driver.session() as session:
            query = """
            MERGE (w:Work {id: $paper_id})
            ...
            """
            await session.run(query, paper_id=paper_id, topic_id=topic_id, topic_name=topic_name, score=score)

    @neo4j_operation_with_retry()
    async def merge_author_topic_relationship(self, author_id: str, topic_id: str, topic_name: str, paper_count: int):
        async with self.driver.session() as session:
            query = """
            MERGE (a:Author {id: $author_id})
            MERGE (t:Topic {id: $topic_id})
            ...
            """
            await session.run(query, author_id=author_id, topic_id=topic_id, topic_name=topic_name, paper_count=paper_count)

    @neo4j_operation_with_retry()
    async def merge_author_institution_relationship(self, author_id: str, institution_id: str, 
    ...
    @neo4j_operation_with_retry()
    async def merge_author_node(self, author_id: str, display_name: str, orcid: Optional[str], 
    ...
    @neo4j_operation_with_retry()
    async def get_coauthor_network(self, author_id: str, limit: int = 50):
        """
        ...
        async with self.driver.session() as session:
            ...
            LIMIT $limit
            """
            result = await session.run(query, author_id=author_id, limit=limit)
            return await result.graph()

    @neo4j_operation_with_retry()
    async def get_topic_network(self, author_id: str, limit: int = 50):
        """
    ...
    @neo4j_operation_with_retry()
    async def get_author_topics(self, author_id: str, limit: int = 20):
        """
    ...
    @neo4j_operation_with_retry()
    async def execute_custom_query(self, query: str, params: Dict = None, limit: int = 50):
        """
    ...
    @neo4j_operation_with_retry()
    async def get_node_by_id(self, node_type: str, node_id: str) -> dict:
        """
